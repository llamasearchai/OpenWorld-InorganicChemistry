...
class PaperAgent:
    ...
    def __init__(self):
        ...
        if not self.openai_available and not self.ollama_available:
            logger.warning("Neither OpenAI nor Ollama is configured. Agent functionality will be limited.")
    ...
    def _validate_analysis(self, analysis: str, analysis_type: str) -> bool:
        ...
        if not any(keyword in analysis.lower() for keyword in relevant_keywords):
            return False
        return True

    async def advanced_summarization(self, papers: list[dict[str, Any]], refinement_level: int = 3) -> str:
        ...
        for level in range(1, refinement_level + 1):
            ...
            refined_prompt = f"Refine this analysis with more depth: {analysis}\nFeedback: {feedback}"
            if self.openai_available:
                analysis = await self._analyze_with_openai(papers, "advanced_summary", refined_prompt=refined_prompt)
            elif self.ollama_available:
                analysis = await self._analyze_with_ollama(papers, "advanced_summary", refined_prompt=refined_prompt)
            else:
    ...
    async def analyze_papers(self, papers: list[dict[str, Any]], analysis_type: str = "summary") -> str:
        ...
        try:
            ...
            raise AgentError(f"Paper analysis failed: {e}", details={"papers_count": len(papers), "type": analysis_type})

    async def _analyze_with_openai(self, papers: list[dict[str, Any]], analysis_type: str, refined_prompt: str = None) -> str:
        ...
        try:
            ...
            papers_text = "\n".join([
                f"- {p.get('title', 'Unknown')} by {', '.join(p.get('authors', ['Unknown']))} ({p.get('date', 'Unknown')})"
                for p in papers
            ])

            prompt = refined_prompt or f"""Analyze the following papers and provide a {analysis_type}:
...
Analysis:"""
            ...
            raise AgentError(f"OpenAI analysis failed: {e}", details={"prompt_length": len(prompt)})

    async def _analyze_with_ollama(self, papers: list[dict[str, Any]], analysis_type: str, refined_prompt: str = None) -> str:
...
Analysis:"""
            ...
            raise AgentError(f"Ollama analysis failed: {e}", details={"prompt_length": len(prompt)})
...
class OllamaModel:
    ...
    async def generate(self, prompt: str, context: str = "") -> str:
        ...
        try:
            ...
            raise AgentError(f"Ollama generation failed: {e}", details={"prompt_length": len(prompt)})


async def run_agent(prompt: str, **kwargs: Any) -> str:
    """Run the AI agent with the given prompt."""
    logger_ctx = logger.bind(prompt=prompt[:50] + "..." if len(prompt) > 50 else prompt, kwargs=kwargs)
    try:
        if not AGENTS_AVAILABLE:
            # Minimal direct OpenAI fallback using responses client
            # Moved import outside try-except to ensure AsyncOpenAI is always defined if openai is installed
            ...
            try:
                ...
                logger_ctx.warning(f"Falling back to Ollama due to OpenAI error: {e}")
                return await OllamaModel().generate(prompt, context="agent_fallback")
        else:
            # Use OpenAI Agents SDK if available
            try:
                from openai import AsyncOpenAI
                ...
                logger_ctx.warning(f"Falling back to Ollama due to OpenAI Agents error: {e}")
                return await OllamaModel().generate(prompt, context="agent_fallback")
    except AgentError:
        raise
    except Exception as e:
        logger_ctx.error(f"Agent execution failed: {e}")
        raise AgentError(f"Agent execution failed: {e}", details={"prompt_length": len(prompt)})
